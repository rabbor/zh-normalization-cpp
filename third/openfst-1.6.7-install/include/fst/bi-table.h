...
public:
  ...
  CompactHashBiTable(const CompactHashBiTable<I, T, H, E, HS> &table)
      : hash_func_(new H(*table.hash_func_)),
        hash_equal_(new E(*table.hash_equal_)), compact_hash_func_(*this),
  ...
  I FindId(const T &entry, bool insert = true) {
    ...
    if (insert) {
      ...
      if (!result.second)
        return *result.first; // Already exists.
...
// takes its ownership.
template <class I, class T, class FP> class VectorBiTable {
public:
...
public:
  ...
  VectorHashBiTable(const VectorHashBiTable<I, T, S, FP, H, HS> &table)
      : selector_(new S(*table.selector_)), fp_(new FP(*table.fp_)),
        h_(new H(*table.h_)), id2entry_(table.id2entry_), fp2id_(table.fp2id_),
        hash_func_(*this), hash_equal_(*this),
...
// the hash function.
template <class I, class T, class F> class ErasableBiTable {
public:
